...
class _LoginScreenState extends State<LoginScreen>
  ...
  final List<TextEditingController> _resetOtpControllers = List.generate(6, (_) => TextEditingController());
  final List<FocusNode> _resetOtpFocusNodes = List.generate(6, (_) => FocusNode());
  ...
  try {
    ...
    try {
      ...
      if (currentUser != null && currentUser.emailConfirmedAt == null) {
        ...
        _showSuccessSnackBar('Please verify your email. We\'ve sent you a verification code.');
      } else {
...
Future<void> _resendEmailOTP() async {
  try {
    ...
    if (_pendingVerificationEmail != null && _isDummyEmail(_pendingVerificationEmail!)) {
      _showDummyEmailDialog();
      return;
    }
    
    await authService.sendVerificationToExistingUser(email: _pendingVerificationEmail!);
    _showSuccessSnackBar('Verification code sent again! Check your inbox and spam folder.');
    _startResendTimer();
...
bool _isDummyEmail(String email) {
  ...
  if (RegExp(r'^(test|dummy|fake|example)\d*@(test|dummy|fake|example)\d*\.(com|org|net)$')
      .hasMatch(lowerEmail)) {
    return true;
  }
...
Future<void> _sendPasswordResetOTP() async {
  ...
  try {
    ...
    if (userStatus?['registration_type'] == 'simple' && !userStatus?['is_verified']) {
...
Future<void> _updateNewPassword() async {
  ...
  try {
    ...
    _showSuccessSnackBar('Password updated successfully! Please sign in with your new password.');
...
void _showUpgradeToVerifiedDialog(String email) {
  ...
  final double maxWidth = isWeb ? 500 : screenWidth * 0.92; // Same as your login form
...
Future<void> _upgradeToVerified(String email) async {
  ...
  if (_isDummyEmail(email)) {
    _showErrorSnackBar('Please use a valid email address. Dummy emails cannot receive verification codes.');
    return;
  }
  ...
  try {
    ...
    _startResendTimer();
    _showSuccessSnackBar('Verification code sent! Check your inbox and spam folder.');
    ...
    if (e.toString().toLowerCase().contains('rate limit')) {
      ...
      errorMessage += 'Account not found. Please ensure you\'re using a real email address.';
    } else {
      errorMessage += 'Please ensure you\'re using a real email address and check your internet connection.';
    }
      ...
      children: [
        ...
        TextButton(
          ...
          child: Text(
            ...
            style: GoogleFonts.poppins(
              color: _canResendResetOTP ? Colors.red.shade600 : Colors.grey.shade500,
            ),
          ),
        ),
...
Future<bool> _onWillPop() async {
  ...
  if (_showOTPVerification || _showForgotPassword || _showPasswordResetOTP || _showNewPasswordForm) {
    setState(() {
      _showOTPVerification = false;
      _showForgotPassword = false;
      _showPasswordResetOTP = false;
