...
class SafeSpotDetails {
  static void showSafeSpotDetails({
    ...
    final DateTime createdTime = DateTime.parse(safeSpot['created_at']).toLocal();
...
try {
  ...
  static void _showDeleteDialog(BuildContext context, String safeSpotId, VoidCallback onUpdate) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Delete Safe Spot'),
        content: const Text('Are you sure you want to delete this safe spot? This action cannot be undone.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
